    ...
    with open(file, "r") as f:
        ...
        while line:
            ...
            if int(lineDecomposed[1]) == 0:
                ...
                currentActivity.requiredResources = [int(lineDecomposed[2]), int(lineDecomposed[3]),
                                                     int(lineDecomposed[4]), int(lineDecomposed[5])]
...
'''
with open('stateActionsDataset/states.csv', 'wb') as statesCsv_file:
    writer = csv.DictWriter(statesCsv_file)
    writer.writerows(states)

with open('stateActionsDataset/actions.csv', 'wb') as actionsCsv_file:
    writer = csv.DictWriter(statesCsv_file)
...
for i in range(numberOfFilesValidate):
    ...
    meanDurationPerActivitySequenceValidateRecord.append(
        [indexFilesValidate[i], activitySequences[indexFilesValidate[i]].totalDurationMean])
    # Calculate percentage improvement for each activitySequence
    percentageImprovementMinToMeanValidateRecord.append([indexFilesValidate[i], round(100 * (1 - (
                activitySequences[indexFilesValidate[i]].totalDurationMin / activitySequences[
            indexFilesValidate[i]].totalDurationMean)), 4)])
