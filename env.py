...
class runSimulation_input:
    def __init__(self):
        ...
        self.useFutureResourceUtilisation = None
...
def runSimulation(runSimulation_input):
    ...
    neuralNetworkType = runSimulation_input.neuralNetworkType
        ...
        while simulationRunFinished == False:    # if there are some token left in some activities
            ...
            # Xiaolei: policy types "shortest processing time" and "shortest sumDuration including successor"
                ...
                # code for this policy type from: https://github.com/leiiiiii/RCPSP/blob/master/Env.py
                    ...
                    for i in range(len(indexReadyToStartActivitiesInState)):
                        successorActivities = currentActivitySequence.activities[
                            indexReadyToStartActivitiesInState[i]].indexFollowingActivities
                        ...
                        totalDurationIncludeSuccessor[i] = currentActivitySequence.activities[
                                                               indexReadyToStartActivitiesInState[
                                                                   i]].time + totalDurationforSuccessor

                    indexActivitiesGlobal = [-1] * numberOfActivitiesInStateVector
                    indexActivitiesGlobal[
                    0:len(indexReadyToStartActivitiesInState)] = indexReadyToStartActivitiesInState
                    indexActivitiesGlobal_reordered = [x for _, x in
                                                       sorted(zip(totalDurationIncludeSuccessor, indexActivitiesGlobal),
                                                              reverse=True)]
                    activityConversionVector = indexActivitiesGlobal_reordered
            ...
            if trivialDecision == False:
                    ...
                    for i, indexActivity in enumerate(activityConversionVector):
                        ...
                        if indexActivity != -1:
                            ...
                            for j in range(numberOfResources):
                                ...
                                currentActivitySequence.activities[indexActivity].requiredResources[resourceConversionVector[j]] / currentActivitySequence.totalResources[resourceConversionVector[j]]
                ...
                # neuralNetworkType = "2dimensional convnet"
                    ...
                    if trivialDecision == False:
                        ...
                        for i, indexActivity in enumerate(activityConversionVector):
                            if indexActivity != -1:
                                currentState_readyToStartActivities[0, 0 + i * (1 + numberOfResources)] = \
                                    currentActivitySequence.activities[indexActivity].time * rescaleFactorTime
                                for j in range(numberOfResources):
                                    currentState_readyToStartActivities[0, 1 + j + i * (1 + numberOfResources)] = \
                        ...
                        for j in range(numberOfResources):
                            currentState_readyToStartActivities[
                                0, numberOfActivitiesInStateVector + numberOfActivitiesInStateVector * numberOfResources + j] = \
                                currentActivitySequence.availableResources[resourceConversionVector[j]] / \
                                currentActivitySequence.totalResources[resourceConversionVector[j]]
                        ...
                        currentState_readyToStartActivitiesMatrix = np.matmul(
                            currentState_readyToStartActivitiesVertical, currentState_readyToStartActivities)
                ...
                if useFutureResourceUtilisation == True:
                    ...
                    currentState_futureResourceUtilisation_forActive = np.dot(resourceUtilizationMatrix,
                                                                              timeHorizonMatrix)
                    ...
                    if len(indexFollowingActivities) == 0:
                        currentState_futureResourceUtilisation_forFollowing = np.zeros(
                            (numberOfResources, timeHorizon))
                        ...
                        if len(indexFollowingActivities) > 1:
                            ...
                            for i in indexFollowingActivities:
                                currentActivitySequence.activities[i].requiredResources = np.array(
                                    currentActivitySequence.activities[i].requiredResources)
                                resourceUtilizationMatrixforFollowing = currentActivitySequence.activities[
                                    i].requiredResources.reshape((numberOfResources, 1))

                        # currentState_futureResourceUtilisation for following activities generated
                        currentState_futureResourceUtilisation_forFollowing = np.dot(
                            resourceUtilizationMatrixforFollowing, timeHorizonMatrixforFollowing)

                    currentState_futureResourceUtilisation = np.add(
                        currentState_futureResourceUtilisation_forActive,
                        currentState_futureResourceUtilisation_forFollowing)

                    for i in range(numberOfResources):
                        for j in range(timeHorizon):
                            currentState_futureResourceUtilisation[i][j] = \
                                currentState_futureResourceUtilisation[i][j] / currentActivitySequence.totalResources[i]
            ...
            if randomDecisionAtThisStep:
                ...
                if policyType == "neuralNetworkModel" and neuralNetworkType == "1dimensional convnet":
            ...
            for indexActivityToStartLocal in decisions_indexActivity_reordered:
                ...
                if indexActivityToStartGlobal != -1:
                    ...
                    if currentActivity.withToken and currentActivity.idleToken and currentActivity.numberOfCompletedPreviousActivities == currentActivity.numberOfPreviousActivities:
                        ...
                        for i in range(numberOfResources):
                            if currentActivity.requiredResources[i] > \
                                    currentActivitySequence.availableResources[i]:
                                enoughResourcesAreAvailable = False
                                break

                        if enoughResourcesAreAvailable:
                            currentActivitySequence.activities[
                                indexActivityToStartGlobal].idleToken = False  ##########activity starting!!!
                            ...
                            for i in range(numberOfResources):
                                currentActivitySequence.activities[
                                    indexActivityToStartGlobal].seizedResources[i] = currentActivitySequence.activities[
                                    indexActivityToStartGlobal].requiredResources[i]
            ...
            # also distinguish if data is generated for 1dimensional or 2 dimensional convnet (vector vs. matrix as a state)
